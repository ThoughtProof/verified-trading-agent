...
async function runCycle(cycle: number, market: MarketSnapshot, reputation: ReputationWriter | null): Promise<void> {
  ...
  let verification = await verifyDecision(decision, THOUGHTPROOF_API_KEY, marketSituation, market);
  ...
  if (firstBlocked) {
    ...
    try {
      ...
      const revisedVerification = await verifyDecision(revised.decision, THOUGHTPROOF_API_KEY, marketSituation, market);
